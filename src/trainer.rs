...
impl Into<Network> for Snapshot {
...
pub trait Trainer<O: 'static + Optimizer>
where
    Self: IntoIterator<Item = anyhow::Result<f32>> + Iterator<Item = anyhow::Result<f32>>,
{
    ...
    fn save(&self) -> anyhow::Result<()> {
        ...
        (|| -> anyhow::Result<()> {
            fs::write(&path, serde_json::to_string(self.tb())?)?;
            Ok(())
        })()
        .with_context(|| {
            format!(
...
pub struct TrainerBase<O: Optimizer> {
    //constants
    pub config: TrainingConfig,
    pub batch_count: usize,
    ...
    pub save_handler: Box<dyn Fn(u32) -> String>, //called every time a network is saved to provide the file name
}
impl<O: Optimizer> TrainerBase<O> {
...
impl<O: Optimizer> Serialize for TrainerBase<O> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        let mut st = serializer.serialize_struct("TrainerBase", 5)?;
        st.serialize_field("network", self.optimizer.network())?;
...
pub struct BuilderBase<O: Optimizer> {
    //constants
    ...
    pub save_handler: Option<Box<dyn Fn(u32) -> String>>, //called every time a network is saved to provide the file name
}
impl<O: Optimizer> BuilderBase<O> {
...
pub trait TrainerBuilder<O: Optimizer>
where
    Self: Sized,
{
    ...
    fn save_handler(mut self, save_handler: impl Fn(u32) -> String + 'static) -> Self {
